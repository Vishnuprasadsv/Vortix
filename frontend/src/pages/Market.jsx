...
const Market = () => {
  ...
  const getSortedCoins = () => {
    ...
    switch (sortOption) {
      ...
      case "mcap-asc":
        return filtered.sort((a, b) => a.market_cap - b.market_cap);
      default:
        return filtered;
    }
  };
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="relative">
            <button
              ...
              className={`p-3 bg-[#0F1114] border ${
                filterOpen
                  ? "border-primary text-primary"
                  : "border-gray-800 text-gray-400"
              } rounded-lg hover:border-gray-600 hover:text-white transition-colors cursor-pointer`}
            >
              <FaFilter />
            </button>
            ...
            {filterOpen && (
              <div className="absolute top-full right-0 mt-2 w-56 bg-surface border border-gray-700 rounded-lg shadow-xl z-50 p-2">
                <h3 className="text-xs font-bold text-text-muted uppercase px-2 py-1 mb-1">
                  Sort Market
                </h3>
                {[
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm rounded transition-colors flex items-center justify-between cursor-pointer ${
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left border-b border-gray-800">
                  <th className="pb-4 pl-4 text-xs font-bold text-gray-500 uppercase tracking-wider w-16 hidden md:table-cell">
                    #
                  </th>
                  <th className="pb-4 text-xs font-bold text-gray-500 uppercase tracking-wider pl-4 md:pl-0">
                    Coin
                  </th>
                  <th className="pb-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">
                    Price
                  </th>
                  <th className="pb-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right pr-4 md:pr-0">
                    24H
                  </th>
                  <th className="pb-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right hidden md:table-cell">
                    M.Cap
                  </th>
                  <th className="pb-4 pr-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right w-32 hidden md:table-cell">
                    Action
                  </th>
                </tr>
              </thead>
                  ...
                  <React.Fragment key={coin.id}>
                    <tr
                      className={`group hover:bg-white/5 transition-colors cursor-pointer ${
                        expandedCoinId === coin.id ? "bg-white/5" : ""
                      }`}
                      onClick={() => toggleExpand(coin.id)}
                    >
                      <td className="py-4 pl-4 text-gray-500 font-medium text-sm hidden md:table-cell">
                        {index + 1}
                      </td>
                          ...
                          >
                            <div>
                              ...
                              <p className="text-gray-400 text-sm leading-relaxed">
                                {coin.name} ({coin.symbol.toUpperCase()}) is a
                                cryptocurrency with a market cap of $
                                {coin.market_cap.toLocaleString()}.
                              </p>
                            </div>
