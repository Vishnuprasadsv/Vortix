...
export const MarketSentiment = () => {
  ...
  return (
    <div className="bg-surface rounded-xl border border-gray-800 p-6 flex flex-col justify-between">
      <div>
        <h3 className="text-xs font-bold text-text-muted uppercase mb-1">
          Market Sentiment
        </h3>
      ...
      <div className="space-y-4">
        {signals.map((coin) => (
          <div key={coin.id} className="flex justify-between items-center">
            ...
            <span
              className={`px-3 py-1 text-xs font-bold rounded border ${coin.bg} ${coin.color}`}
            >
              {coin.signal}
            </span>
          </div>
...
export const Watchlist = ({ onCoinSelect, selectedCoinId, coins = [] }) => {
  ...
  return (
    <div
      className={`bg-surface rounded-xl border border-gray-800 p-6 flex flex-col ${
      ...
      <div className="space-y-4 overflow-y-auto max-h-[500px] custom-scrollbar">
        {displayedCoins.map((coin, idx) => (
          <div
            ...
            className={`flex justify-between items-center p-3 rounded-lg hover:bg-white/5 transition-colors cursor-pointer border ${
          ...
          >
            ...
            <div className="text-right">
              ...
              <div
                className={`${
                  (coin.price_change_percentage_24h || coin.change || 0) >= 0
                    ? "text-green-500"
                    : "text-red-500"
                } text-xs`}
              >
                {(coin.price_change_percentage_24h || coin.change || 0) > 0
                  ? "+"
                  : ""}
                {(coin.price_change_percentage_24h || coin.change || 0).toFixed(
                  2
                )}
                %
              </div>
