...
import { updatePortfolioPrices, sellAsset } from '../redux/slices/portfolioSlice';
...
const getIcon = (symbol, color) => {
    switch (symbol) {
        ...
        case 'ETH': return <FaEthereum className="text-blue-500 text-2xl" />;
        default: return <div className="font-bold text-lg" style={{ color: color }}>{symbol?.charAt(0)}</div>;
    }
}
            ...
            >
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        ...
                        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4 w-full md:w-auto">
                            <div className="bg-black/40 border border-gray-800 rounded-lg px-6 py-3 text-center md:text-left">
                                <div className="text-xs text-center text-gray-500 font-bold uppercase tracking-wider mb-1">Total Balance</div>
                            ...
                            >
                                <FaWallet className="mr-2" /> Withdraw <span className="inline md:inline ml-1">Funds</span>
                            </Button>
                        </div>
                    </div>
                </div>
                ...
                {assets.length === 0 ? (
                    <div className="bg-[#0F1114] border border-gray-800 border-dashed rounded-xl p-12 min-h-[400px] flex flex-col items-center justify-center text-center">
                        ...
                        <h3 className="text-lg font-medium text-gray-400">No assets found in your portfolio.</h3>
                    </div>
                ) : (
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="text-left border-b border-gray-800 bg-white/5">
                                        <th className="py-4 pl-6 text-xs font-bold text-gray-500 uppercase">Asset</th>
                                        <th className="py-4 text-xs font-bold text-gray-500 uppercase text-right">Balance</th>
                                        <th className="py-4 text-xs font-bold text-gray-500 uppercase text-right">Avg. Price</th>
                                        <th className="py-4 text-xs font-bold text-gray-500 uppercase text-right">Value</th>
                                        <th className="py-4 pr-6 text-xs font-bold text-gray-500 uppercase text-right w-24">Action</th>
                                    </tr>
                                </thead>
                                ...
                                <div key={asset.id} className="p-4 flex flex-col gap-4">
                                    ...
                                    <div className="flex items-center justify-between bg-black/20 p-3 rounded-lg border border-gray-800/50">
                                        <div className="flex flex-col">
                                            <span className="text-xs text-gray-500 uppercase">Avg. Price</span>
